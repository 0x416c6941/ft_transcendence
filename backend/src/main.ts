...
if (!sslKeyPath) {
  throw new Error(
    "process.env.BACKEND_FASTIFY_SSL_KEY_PATH isn't a valid SSL key."
  );
}
const sslCertPath = process.env.BACKEND_FASTIFY_SSL_CRT_PATH;
if (!sslCertPath) {
  throw new Error(
    "process.env.BACKEND_FASTIFY_SSL_CRT_PATH isn't a valid SSL key."
  );
}
...
if (!dbVolPath) {
  throw new Error(
    "process.env.BACKEND_CONTAINER_DB_VOL_PATH isn't a valid path."
  );
}
