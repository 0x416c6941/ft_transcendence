...
export default class TetrisRoomsView extends AbstractView {
    ...
    async getHtml(): Promise<string> {
        return `
            <main class="flex-1 min-h-0 flex flex-col justify-center items-center bg-neutral-200 dark:bg-neutral-900 p-4">
                <h1 class="txt-light-dark-sans text-3xl mb-6">Tetris Tournament Room - ${APP_NAME}</h1>
                    ...
                    <div class="bg-white dark:bg-neutral-800 p-8 rounded-lg shadow-lg w-full max-w-md">
                        ...
                        <form id="room-form" class="space-y-6">
                            ...
                            <button
                                type="button"
                                id="enter-room-btn"
                                class="w-full bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-md font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-orange-500 opacity-50 cursor-not-allowed"
                                disabled
                            >
                                Enter Room
                            </button>
    ...
    setDocumentTitle(): void {
        document.title = `${APP_NAME} - Tetris Tournament Room`;
    }
    ...
    private loadRoomsList(): void {
        ...
        this.socket.emit('get_tetris_tournament_rooms', (rooms: Array<{ name: string; playerCount: number; maxPlayers: number; hasPassword: boolean; status: string }>) => {
    ...
    private enterRoom(name: string, password: string): void {
        ...
        this.socket.emit('enter_tetris_tournament_room', { name, password }, (response: { success: boolean; roomId?: string; error?: string }) => {
            this.setButtonEnabled(true);

            if (response.success && response.roomId) {
                this.router.navigate(`/tetris-tournament/${response.roomId}`);
            } else {
