...
<main class="flex flex-1 items-center justify-center bg-neutral-100 dark:bg-neutral-900">
	...
	private validateAll(
		...
		displayEl: HTMLInputElement,
		emailEl: HTMLInputElement,
		passwordEl: HTMLInputElement): boolean
		{
	...
	private hasChanges(
  	...
  	async setup(): Promise<void> {
    		...
    		if (!auth.isAuthed()) {
                ...
                if (!form || !msg || !logoutBtn || !deleteBtn || !updateBtn || !editBtn || !avatarImg) return;
		...
		try {
			...
			// Backend should always provide a valid URL, but ignore errors just in case
			// Optionally: avatarImg.src = "/default_avatar.webp";
		}
	...
	form.addEventListener("submit", async (event) => {
		...
		const {username, display_name, email, password } = this.readSanitized(usernameEl, displayNameEl, emailEl, passwordEl);
		...
		try {
			...
			msg.className = "text-red-500 text-sm text-center mt-2";
			// Stay in edit mode for correction
			setEditing(true);
			updateSaveState();
