  ...
  async getHtml(): Promise<string> {
    return `
      <main class="h-screen flex flex-col justify-center items-center bg-neutral-100 dark:bg-neutral-900">
        <h1 class="text-3xl mb-6">Login to ${APP_NAME}</h1>
        <form id="login-form" class="flex flex-col gap-4 w-64" novalidate>
          <input id="username" type="text" placeholder="Username" class="p-2 rounded border" />
          <input id="password" type="password" placeholder="Password" class="p-2 rounded border" />
        ...
        <div class="mt-6">
          <a href="/" data-link class="underline">Back to Home</a>
        </div>
      </main>
    `;
  }
